...
import { getUserId } from "./clerk-server"; // FIXED: Use the safety helper
import { revalidatePath } from "next/cache";
import { supabase } from "./db";

async function checkAdminStatus() {
  const userId = await getUserId(); // FIXED
...
export async function banUserAction(targetUserId, currentStatus) {
  const adminId = await getUserId(); // FIXED
...
export async function sendMessageAction(receiverId, content, subject = null) {
  const senderId = await getUserId(); // FIXED
